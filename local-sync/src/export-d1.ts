...
// --------------------------------------------------
// Export D1 : D1 から `tracks` テーブルを全件取得して JSON ファイルに出力する
// --------------------------------------------------

console.log(`[${jst()}] Export D1 : Start`);
...
/** D1 から全楽曲を取得する */
const fetchD1Tracks = async (d1ApiToken: string): Promise<Result<Array<D1Track>>> => {
  try {
    ...
    if(queryResult?.result[0]?.success == null) {
      ...
      return { error: 'Failed To Fetch D1 Tracks' };  // 早期 `return` の目印になればいいだけなので内容はテキトー
    }
    if(queryResult?.result[0]?.success === false) {
      errorLog('D1 API の `success` レスポンスが `false` でした');
      return { error: 'Failed To Fetch D1 Tracks' };
    }
...
const main = async (): Promise<void> => {
  ...
  // D1 から全楽曲を取得する
  const d1TracksResult = await fetchD1Tracks(d1ApiToken);
  ...
  d1Tracks.forEach(d1Track => {
    ...
    if(parsed.success) {
      result.d1_tracks.push(parsed.data);
      result.summary.d1_tracks++;
    }
    else {
  ...
  console.log(`[${jst()}]   Zod パースを通過した楽曲数 : ${result.summary.d1_tracks}`);
  ...
  if(result.summary.d1_tracks === 0 || result.d1_tracks.length === 0) {  // 正常なら早期 `return` しているはず・ココで発生している場合は全 Zod パースに失敗している可能性アリ
  ...
  // 件数の不一致がないかチェックする
  const totalCount = result.summary.d1_tracks + result.summary.invalid_tracks;
  if(result.summary.total_tracks !== totalCount) {
    errorLog(`総処理した楽曲数カウントが不一致です・実装誤りの恐れがあります : Total ${result.summary.total_tracks}・Valid ${result.summary.d1_tracks}・Invalid ${result.summary.invalid_tracks}・差異 ${result.summary.total_tracks - totalCount}`);
  }
  if(result.summary.d1_tracks !== result.d1_tracks.length) {
    errorLog(`Zod パースを通過した D1 楽曲数カウントが不一致です・実装誤りの恐れがあります : Count ${result.summary.d1_tracks}・実数 ${result.d1_tracks.length}・差異 ${result.summary.d1_tracks - result.d1_tracks.length}`);
  }
...
(async () => {
  try {
    ...
    writeResult();
    console.log(`[${jst()}] Export D1 : Finished`);
  }
})();
